...
class AbstractPrinting:
    ...
    def show_dialog(self) -> None:
        ...
        diag = printsupport.QPrintDialog(self._tab)
        if utils.is_mac:
            # For some reason we get a segfault when using open() on macOS
            ret = diag.exec()
            if ret == widgets.QDialog.DialogCode.Accepted:
                do_print()
        else:
            diag.open(do_print)
...
class AbstractScroller(core.QObject):
    ...
    def _init_widget(self, widget: widgets.QWidget) -> None:
        self._widget = widget
...
class AbstractTabPrivate:
    ...
    def event_target(self) -> widgets.QWidget:
...
class AbstractTab(widgets.QWidget):
    ...
    def _set_widget(self, widget: widgets.QWidget) -> None:
